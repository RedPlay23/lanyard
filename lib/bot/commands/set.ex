      ...
      {false} ->
        case Lanyard.KV.Interface.set(payload["author"]["id"], key, value) do
          ...
          _ ->
            DiscordApi.send_message(
              payload["channel_id"],
              ":white_check_mark: `#{key}` was set. View it with `#{Application.get_env(:lanyard, :command_prefix)}get #{key}` or go to https://discorduserapi.rajatcj.com.np/v1/users/#{payload["author"]["id"]}"
            )
        end
    end
